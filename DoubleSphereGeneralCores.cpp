...
int main()
{
	string path="C:\\Users\\shadowcage72\\Eclipse\\TestMat3\\";
	...
	double L=9/meterconv*1.25;//Length
	double DT=L/120;//L/60;//DeltaT

//	for(int number=27;number++;number<2)
//	{
	int number=1;
		double separation=.68*L/1.25;


	int NT=600000;//Iterations
	int NG=32;//Number of Grid Points
	double NG1=NG;
	double ndensity=10000000000*meterconv*meterconv*meterconv;
	double totalparticles=2*ndensity*L*L;//Total number of electrons in simulation


	//At least 25 per cell is ideal

	int N=234400;//Number of Particles
...
//Preallocate memory
	...
	double syc1=.5*L-separation/2;//Sphere 1 y center
	double sr1=.04*L/1.25;//Sphere 1 radius


	//sphere2
	double sxc2=.5*L;//Sphere 2 x center
	double syc2=.5*L+separation/2;//Sphere 2 y center
	double sr2=.04*L/1.25;//Sphere 2 radius
...
int i=0;
	// update xp
#pragma omp parallel sections num_threads(Cores)
		{
		for(i=0;i<Cores;i++)
		{
		#pragma omp section
			{
				for(n=i*N/Cores;n<(i+1)*N/Cores;n++)
					{
						xp(n)+=vxp(n)*DT/2;
						yp(n)+=vyp(n)*DT/2;
					}
		...
		if(xp(n)==0)
		{
			if((N-n-1)>0)
			{
				...
				if(n<N)
				{
					...
					massmat(n)=massmat(N);
					c(n)=c(N);
					InitialVelocity(n)=InitialVelocity(N);
					Pot(n)=Pot(N);
					xp(N)=0;
					yp(N)=0;
					vxp(N)=0;
...
#pragma omp parallel sections num_threads(Cores)
		...
		#pragma omp section
			{
				for(n=i*NG*NG/Cores;n<(i+1)*NG*NG/Cores;n++)
					{
						rho(n)=0;
					}
				}
				...
				for(n=i*N/Cores;n<(i+1)*N/Cores;n++)
					{
							if(index2(g(n,0),g(n,1),NG)<NG*NG/4)
							{
								vyp(n)+=(Phiseg1(index2(g(n,0),g(n,1),NG)))*(fraz(n,0)+fraz(n+N,0))/dx*emat(n)*DT/massmat(n);
								vxp(n)+=(Phiseg1(index2(g(n,0),g(n,1),NG)))*(fraz(n,0)+fraz(n,1))/dx*emat(n)*DT/massmat(n);
							}
							else if(index2(g(n,0),g(n,1),NG)<NG*NG/2)
							{
								vyp(n)+=(Phiseg2(index2(g(n,0),g(n,1),NG)-NG*NG/4))*(fraz(n,0)+fraz(n+N,0))/dx*emat(n)*DT/massmat(n);
								vxp(n)+=(Phiseg2(index2(g(n,0),g(n,1),NG)-NG*NG/4))*(fraz(n,0)+fraz(n,1))/dx*emat(n)*DT/massmat(n);
							}
							else if(index2(g(n,0),g(n,1),NG)<NG*NG*3/4)
							{
								vyp(n)+=(Phiseg3(index2(g(n,0),g(n,1),NG)-NG*NG/2))*(fraz(n,0)+fraz(n+N,0))/dx*emat(n)*DT/massmat(n);
								vxp(n)+=(Phiseg3(index2(g(n,0),g(n,1),NG)-NG*NG/2))*(fraz(n,0)+fraz(n,1))/dx*emat(n)*DT/massmat(n);
							}
							else
							{
								vyp(n)+=(Phiseg4(index2(g(n,0),g(n,1),NG)-NG*NG/4*3))*(fraz(n,0)+fraz(n+N,0))/dx*emat(n)*DT/massmat(n);
								vxp(n)+=(Phiseg4(index2(g(n,0),g(n,1),NG)-NG*NG/4*3))*(fraz(n,0)+fraz(n,1))/dx*emat(n)*DT/massmat(n);
							}

							if(index2(g(n,0),g(N+n,1),NG)<NG*NG/4)
							{
								vyp(n)+=(-Phiseg1(index2(g(n,0),g(N+n,1),NG)))*(fraz(n,0)+fraz(n+N,0))/dx*emat(n)*DT/massmat(n);
								vxp(n)+=(Phiseg1(index2(g(n,0),g(N+n,1),NG)))*(fraz(n+N,0)+fraz(n+N,1))/dx*emat(n)*DT/massmat(n);
							}
							else if(index2(g(n,0),g(N+n,1),NG)<NG*NG/2)
							{
								vyp(n)+=(-Phiseg2(index2(g(n,0),g(N+n,1),NG)-NG*NG/4))*(fraz(n,0)+fraz(n+N,0))/dx*emat(n)*DT/massmat(n);
								vxp(n)+=(Phiseg2(index2(g(n,0),g(N+n,1),NG)-NG*NG/4))*(fraz(n+N,0)+fraz(n+N,1))/dx*emat(n)*DT/massmat(n);
							}
							else if(index2(g(n,0),g(N+n,1),NG)<NG*NG*3/4)
							{
								vyp(n)+=(-Phiseg3(index2(g(n,0),g(N+n,1),NG)-NG*NG/2))*(fraz(n,0)+fraz(n+N,0))/dx*emat(n)*DT/massmat(n);
								vxp(n)+=(Phiseg3(index2(g(n,0),g(N+n,1),NG)-NG*NG/2))*(fraz(n+N,0)+fraz(n+N,1))/dx*emat(n)*DT/massmat(n);
							}
							else
							{
								vyp(n)+=(-Phiseg4(index2(g(n,0),g(N+n,1),NG)-NG*NG/4*3))*(fraz(n,0)+fraz(n+N,0))/dx*emat(n)*DT/massmat(n);
								vxp(n)+=(Phiseg4(index2(g(n,0),g(N+n,1),NG)-NG*NG/4*3))*(fraz(n+N,0)+fraz(n+N,1))/dx*emat(n)*DT/massmat(n);
							}

							if(index2(g(N+n,0),g(n,1),NG)<NG*NG/4)
							{
								vyp(n)+=(Phiseg1(index2(g(N+n,0),g(n,1),NG)))*(fraz(n,1)+fraz(n+N,1))/dx*emat(n)*DT/massmat(n);
								vxp(n)+=(-Phiseg1(index2(g(N+n,0),g(n,1),NG)))*(fraz(n,0)+fraz(n,1))/dx*emat(n)*DT/massmat(n);
							}
							else if(index2(g(N+n,0),g(n,1),NG)<NG*NG/2)
							{
								vyp(n)+=(Phiseg2(index2(g(N+n,0),g(n,1),NG)-NG*NG/4))*(fraz(n,1)+fraz(n+N,1))/dx*emat(n)*DT/massmat(n);
								vxp(n)+=(-Phiseg2(index2(g(N+n,0),g(n,1),NG)-NG*NG/4))*(fraz(n,0)+fraz(n,1))/dx*emat(n)*DT/massmat(n);
							}
							else if(index2(g(N+n,0),g(n,1),NG)<NG*NG*3/4)
							{
								vyp(n)+=(Phiseg3(index2(g(N+n,0),g(n,1),NG)-NG*NG/2))*(fraz(n,1)+fraz(n+N,1))/dx*emat(n)*DT/massmat(n);
								vxp(n)+=(-Phiseg3(index2(g(N+n,0),g(n,1),NG)-NG*NG/2))*(fraz(n,0)+fraz(n,1))/dx*emat(n)*DT/massmat(n);
							}
							else
							{
								vyp(n)+=(Phiseg4(index2(g(N+n,0),g(n,1),NG)-NG*NG/4*3))*(fraz(n,1)+fraz(n+N,1))/dx*emat(n)*DT/massmat(n);
								vxp(n)+=(-Phiseg4(index2(g(N+n,0),g(n,1),NG)-NG*NG/4*3))*(fraz(n,0)+fraz(n,1))/dx*emat(n)*DT/massmat(n);
							}

							if(index2(g(N+n,0),g(N+n,1),NG)<NG*NG/4)
							{
								vyp(n)+=(-Phiseg1(index2(g(N+n,0),g(N+n,1),NG)))*(fraz(n,1)+fraz(n+N,1))/dx*emat(n)*DT/massmat(n);
								vxp(n)+=(-Phiseg1(index2(g(N+n,0),g(N+n,1),NG)))*(fraz(n+N,0)+fraz(n+N,1))/dx*emat(n)*DT/massmat(n);
							}
							else if(index2(g(N+n,0),g(N+n,1),NG)<NG*NG/2)
							{
								vyp(n)+=(-Phiseg2(index2(g(N+n,0),g(N+n,1),NG)-NG*NG/4))*(fraz(n,1)+fraz(n+N,1))/dx*emat(n)*DT/massmat(n);
								vxp(n)+=(-Phiseg2(index2(g(N+n,0),g(N+n,1),NG)-NG*NG/4))*(fraz(n+N,0)+fraz(n+N,1))/dx*emat(n)*DT/massmat(n);
							}
							else if(index2(g(N+n,0),g(N+n,1),NG)<NG*NG*3/4)
							{
								vyp(n)+=(-Phiseg3(index2(g(N+n,0),g(N+n,1),NG)-NG*NG/2))*(fraz(n,1)+fraz(n+N,1))/dx*emat(n)*DT/massmat(n);
								vxp(n)+=(-Phiseg3(index2(g(N+n,0),g(N+n,1),NG)-NG*NG/2))*(fraz(n+N,0)+fraz(n+N,1))/dx*emat(n)*DT/massmat(n);
							}
							else
							{
								vyp(n)+=(-Phiseg4(index2(g(N+n,0),g(N+n,1),NG)-NG*NG/4*3))*(fraz(n,1)+fraz(n+N,1))/dx*emat(n)*DT/massmat(n);
								vxp(n)+=(-Phiseg4(index2(g(N+n,0),g(N+n,1),NG)-NG*NG/4*3))*(fraz(n+N,0)+fraz(n+N,1))/dx*emat(n)*DT/massmat(n);
							}


//								tempvel=sqrt(vxp(n)*vxp(n)+vyp(n)*vyp(n));
//							expvel=sqrt(InitialVelocity(n)*InitialVelocity(n)+2*Pot(n));
//							vyp(n)=vyp(n)*expvel/tempvel;
//							vxp(n)=vxp(n)*expvel/tempvel;
						}
					}
					}
				}
	cout << "\nit="<<it;
	cout << "\nPhic1(V)="<<Volt1(it)*510999;
	cout << "\nPhic2(V)="<<Volt2(it)*510999;
