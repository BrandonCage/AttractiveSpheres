...
int main()
{
	...
	double NG1=NG;
	double ndensity=500000000*meterconv*meterconv*meterconv;
	double totalparticles=2*ndensity*L*L;//Total number of electrons in simulation


	for(int number=46;number++;number<40)
	{
...
if(it>299999)
{
	Phi1allit=(it-300000)*Phi1allit/(it-299999)+Phi1/(it-299999);
}
			...
			#pragma omp section
						{
							for(n=0;n<N/4;n++)
							{
								g(n,0)=floor(xp(n)/dx-1);
								g(n,1)=floor(yp(n)/dx-1);
								g(N+n,0)=(floor(xp(n)/dx));
								g(N+n,1)=(floor(yp(n)/dx));
							}
						}
			#pragma omp section
						{
	...
	#pragma omp section
				{
					for(n=0;n<N/4;n++)
					{
						fraz(n,0)=(1-(xp(n)/dx-g(n,0)-1))*(1-(yp(n)/dx-g(n,1)-1));
						fraz(n,1)=(xp(n)/dx-g(n,0)-1)*(1-(yp(n)/dx-g(n,1)-1));
						fraz(N+n,0)=(1-(xp(n)/dx-g(n,0)-1))*(yp(n)/dx-g(n,1)-1);
						fraz(N+n,1)=(xp(n)/dx-g(n,0)-1)*(yp(n)/dx-g(n,1)-1);
					}
				}
	#pragma omp section
				{
			...
			#pragma omp section
						{
							for(n=0;n<NG*NG/4;n++)
							{
								        in1=n+NG+1;//Index of right node
								        in2=n+NG;//Index of left node
								        in3=n+1;//Index of right node
								        in4=n;//Index of left node
								        if(in2>NG*NG-1)
								        {
								        	in2=in2-NG*NG;
								        }
								        ...
								        Ey(n)=(Phi(in1)+Phi(in3)-Phi(in2)-Phi(in4))/2/dx;
								        Ex(n)=(Phi(in1)-Phi(in3)+Phi(in2)-Phi(in4))/2/dx;
								}
						}
			#pragma omp section
						{
